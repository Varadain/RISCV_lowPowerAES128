...
`else
    ...
    //   sensor read -> RAM -> DMA copy -> AES-CTR -> UART -> IRQ -> sleep
    // It also starts the SPI sensor interface and reads the power counters.
    // -------------------------------------------------------------------------
    task automatic run_full_soc_pipeline_integration_test();
        ...
        begin
            ...
            dut.u_if_stage.u_instr_mem.rom[p++] = enc_itype(12'd15,  5'd0, 3'b000, 5'd7, 7'b0010011); // x7 = enable all IRQs
            dut.u_if_stage.u_instr_mem.rom[p++] = enc_stype(12'd4,   5'd7, 5'd3, 3'b010, 7'b0100011); // INTC.IRQ_ENABLE = 0xf
            ...
            check_and_report("FULL-SOC", "IRQ_PENDING", "AES/UART/sensor/DMA pending bits captured", dut.u_id_stage.u_reg_file.regs[21] & 32'hf, 32'hf);
    ...
    initial begin
        ...
        if ($test$plusargs("FULL_SOC_SCENARIO_ONLY")) begin
