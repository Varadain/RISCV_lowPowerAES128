            ...
            if (start) begin
                ...
                round_key  <= key;
					   round      <= 4'd1;
				            ...
				            else if ((round < 4'd10) && (round != 4'd0)) begin
...
module sub_bytes(
    input  logic [127:0] in_i,
    output logic [127:0] out_o
);
    genvar byte_idx;
    generate
	  for (byte_idx = 0; byte_idx < 16; byte_idx = byte_idx + 1) begin : gen_sbox
            aes_sbox u_aes_sbox (
                .a(in_i[8*byte_idx +: 8]),
                .d(out_o[8*byte_idx +: 8])
            );
        end
    endgenerate
endmodule
module shift_rows(
    ...
    assign out_o = {
        in_i[127:120], in_i[87:80],   in_i[47:40],   in_i[7:0],
        in_i[95:88],   in_i[55:48],   in_i[15:8],    in_i[103:96],
        in_i[63:56],   in_i[23:16],   in_i[111:104], in_i[71:64],
        in_i[31:24],   in_i[119:112], in_i[79:72],   in_i[39:32]
    };
endmodule
module mix_columns(
    input  logic [127:0] in_i,
    ...
    genvar col_idx;
    generate
	    for (col_idx = 0; col_idx < 4; col_idx = col_idx + 1) begin : gen_mix_col
            mix_col u_mix_col (
                .a(in_i[32*col_idx +: 32]),
                .y(out_o[32*col_idx +: 32])
            );
        end
    endgenerate
endmodule
module mix_col(input  logic [31:0] a,
               output logic [31:0] y);
...
module key_expand(
  input  logic [127:0] key_i,
    ...
    logic [31:0] w0,w1,w2,w3;
	  assign {w0, w1, w2, w3} = key_i;

    logic [31:0] subword;
	 aes_sbox u_sbox_0 (.a(w3[23:16]), .d(subword[31:24]));
    aes_sbox u_sbox_1 (.a(w3[15:8]),  .d(subword[23:16]));
    aes_sbox u_sbox_2 (.a(w3[7:0]),   .d(subword[15:8]));
    aes_sbox u_sbox_3 (.a(w3[31:24]), .d(subword[7:0]));

    function automatic [31:0] rcon_lut(input [3:0] r);
        case (r)
