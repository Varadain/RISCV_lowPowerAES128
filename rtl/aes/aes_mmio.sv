...
module aes_mmio (
    ...
    // The AES engine is considered enabled only while an operation is active.
    // The primitive still receives the normal system clock; no gated clock is
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // clock enable is disabled.
            if (clk_en_i && write_en_i) begin
                case (reg_offset)
                    OFF_CTRL: begin
                        // CTRL[2] selects ECB(0) or CTR(1).
    ...
    // Combinational custom-instruction and MMIO readback multiplexers.
